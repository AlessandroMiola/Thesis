...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State $k_{i^{*}}$ $\xleftarrow{\text{\$}} \{1, ..., n - 1\}$    \Comment{$n$: elliptic curve group order}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\EndFor
		\If {$e_0 = 0$ or $e_0 \geq n$} \Comment{$n$: elliptic curve group order}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\For {$i \gets 0,\dots,r-1$}
		\State $k_i$ $\xleftarrow{\text{\$}} \{1, ..., n - 1\}$ \Comment{$n$: elliptic curve group order}
		\State $j_{i^{*}}$ $\xleftarrow{\text{\$}} \{0, ..., m_i - 1\}$
		\State $R_{i,j_{i^{*}}} \gets k_iG$
		\If {$((j_{i^{*}}+1)$ \% $m_i )\neq 0$}
		%\State $e_{i,j_{i^{*}}} \gets int(hash(M||R_{i,j_{i^{*}}}||i||j_{i^{*}}))$
		%\State assert $e_{i,j_{i^{*}}} \neq 0$ and $e_{i,j_{i^{*}}} < n$
		\For {$j \gets (j_{i^{*}}+1)$ \% $m_i,\dots,m_i-1$}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\For {$i \gets 0,\dots,r-1$}
		\State $e_{i,0} \gets int(hash(M||e_0||i||0))$
		\If {$e_{i,0} = 0$ or $e_{i,0} \geq n$} \Comment{$n$: elliptic curve group order}
		\State \textbf{return} ``false"
		\EndIf
